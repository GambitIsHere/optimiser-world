...
import { CATEGORY_BY_SLUG, getCategoryByIndex } from '../../lib/categories.js';
...
export default function LivingSubstrate({ items = [], intensity = 'full' }) {
  ...
  useEffect(() => {
    ...
    function layout() {
      ...
      const sorted = [...items].sort((a, b) => (b.upvotes || b.stats?.upvotes || 0) - (a.upvotes || a.stats?.upvotes || 0));

      partitions.forEach((p, i) => {
        const item = sorted[i % Math.max(1, sorted.length)];
        if (!item) return;
        const cat = item.category_obj || CATEGORY_BY_SLUG[item.category] || getCategoryByIndex(0);
        const votes = item.upvotes || item.stats?.upvotes || 0;
        const isHot = votes > 400;
